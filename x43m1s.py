...
async def timershow(ctx):
    ...
    d=data["days"]
    f=data["func"]
    Embed=discord.Embed(title=f"TIMER :: [{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]",description="running timer", color=0xf0e68c)
...
async def run(ctx, app:str):
    ...
    Embed=discord.Embed(title=f'{ctx.content} ', description=f'[{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]', color=0x00ff00)
    Embed.add_field(name=f'{ctx.author.mention} app :: {app}',value='should be open now',inline = False)
    await ctx.channel.send(embed=Embed)
...
async def close(ctx, app:str):
    ...
    Embed=discord.Embed(title=f'{ctx.content}',description=f'[{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]',color=0x00ff00)
    Embed.add_field(name=f'{ctx.author.mention} app :: {app}',value='should be closed now',inline = False)
    await ctx.channel.send(embed=Embed)
@BOT.command()
async def shot(ctx):
...
async def openweb(ctx, url:str):
    ...
    Embed=discord.Embed(title=f'{ctx.content}', description=f'[{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]', color=0x00ff00)
    Embed.add_field(name=f'{ctx.author.mention} website :: {url}',value='should be open now',inline = False)
    await ctx.channel.send(embed=Embed)
@BOT.command()
async def openyt(ctx, qury:str ):
    ...
    Embed=discord.Embed(title=f'{ctx.content}  [{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]', description="----", color=0x00ff00)
    Embed.add_field(name=f'{ctx.author.mention} query :: {qury}',value='should be open in youtube now',inline = False)
    await ctx.channel.send(embed=Embed)
@BOT.command()
async def cd(ctx, path:str=os.getcwd(), skip:str="F", select:str=None):
    ...
    path=path
    if skip=="F":skip=False
    else:skip=True
    select=select

    # print(len(f1),len(f2),len(f3),len(f4),len(f5))
    Embed=discord.Embed(title=f'{ctx.content} ', description=f'[{_time_.day}.{_time_.month} || {_time_.hour}:{_time_.minute}]', color=0x00ff00)
    Embed.add_field(name=f'{ctx.author.mention} path :: {path}',value='Reading now',inline = False)
    await ctx.channel.send(embed=Embed)
